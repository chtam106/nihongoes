...
- **EN:** Mobile Performance sits at ~80-87 (noisy). The bottleneck is NOT fonts or LCP-resource discovery - it is main-thread hydration cost. LCP (~3.7s) is a text `<p>`; FCP/Speed Index are already ~1.1s, but the LCP paint is blocked by hydrating the React + MUI framework (TBT ~310ms, TTI ~6.5s). The largest homepage chunk (~168KB gzip) is the React-DOM + MUI vendor bundle; Lighthouse's "unused JS" there is framework code needed on other routes and CANNOT be tree-shaken.
- **VI:** Performance mobile ở mức ~80-87 (dao động). Điểm nghẽn KHÔNG phải font hay việc phát hiện tài nguyên LCP, mà là chi phí hydrate main-thread. LCP (~3.7s) là một `<p>` text; FCP/Speed Index đã ~1.1s, nhưng LCP bị chặn bởi việc hydrate framework React + MUI (TBT ~310ms, TTI ~6.5s). Chunk lớn nhất trang chủ (~168KB gzip) là bundle React-DOM + MUI; phần "unused JS" là code framework cần cho route khác, KHÔNG tree-shake được.
